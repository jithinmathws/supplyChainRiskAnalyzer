...
class EdgeBottleneckDetector:
    ...
    def _to_weighted_digraph(self):
        ...
        if self.G.is_multigraph():
            ...
            for u, v, data in self.G.edges(data=True):
                weight = data.get("weight", data.get("transport_time", float("inf")))
                edge_key = data.get("edge_id", data.get("transport_mode", "default"))
    ...
    def simulate_edge_failure(self, edge_source, edge_target, edge_key, source_id, target_id):
        ...
        try:
            H, _, baseline_time = self._get_baseline_route(source_id, target_id)
    ...
    def rank_edge_bottlenecks(self, source_id, target_id, only_active_route_edges=False):
        H, baseline_path, _ = self._get_baseline_route(source_id, target_id)

        active_edges = {(baseline_path[i], baseline_path[i + 1]) for i in range(len(baseline_path) - 1)}
        ...
        for u, v, data in H.edges(data=True):
            ...
            edge_key = data.get("edge_key", data.get("transport_mode", data.get("edge_id", "Unknown")))
            results.append(
                self.simulate_edge_failure(
                    edge_source=u,
                    edge_target=v,
                    edge_key=edge_key,
                    source_id=source_id,
    ...
    def summary_report(self, source_id, target_id, only_active_route_edges=False):
        ...
        return {
            ...
            "most_critical_edge": df.iloc[0]["edge_id"] if not df.empty else None,
